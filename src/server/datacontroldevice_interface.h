 ...
 **/
class KWAYLANDSERVER_EXPORT DataControlDeviceInterface : public Resource
{
    Q_OBJECT
public:
    virtual ~DataControlDeviceInterface();

    SeatInterface *seat() const;

    DataControlSourceInterface *selection() const;

    void sendSelection(DataControlDeviceInterface *other);
    void sendClearSelection();

Q_SIGNALS:
    void selectionChanged(KWayland::Server::DataControlSourceInterface*);
    void selectionCleared();

private:
    friend class DataControlDeviceManagerInterface;
    explicit DataControlDeviceInterface(SeatInterface *seat, DataControlDeviceManagerInterface *parent, wl_resource *parentResource);
...
Q_DECLARE_METATYPE(KWayland::Server::DataControlDeviceInterface*)

...
#ifndef K_DOXYGEN
const struct zxdg_exporter_v2_interface XdgExporterUnstableV2Interface::Private::s_interface = {destroyCallback, exportCallback};
#endif
...
void XdgExporterUnstableV2Interface::Private::exportCallback(wl_client *client, wl_resource *resource, uint32_t id, wl_resource *surface)
{
    ...
    connect(e.data(), &XdgExportedUnstableV2Interface::unbound, s->q, [s, handle]() {
        s->exportedSurfaces.remove(handle);
        Q_EMIT s->q->surfaceUnexported(handle);
    });

    // if the surface dies before this, this dies too
    connect(SurfaceInterface::get(surface), &Resource::unbound, s->q, [s, e, handle]() {
        if (e) {
            e->deleteLater();
        }
        s->exportedSurfaces.remove(handle);
...
#ifndef K_DOXYGEN
const struct zxdg_importer_v2_interface XdgImporterUnstableV2Interface::Private::s_interface = {destroyCallback, importCallback};
#endif
...
void XdgImporterUnstableV2Interface::Private::importCallback(wl_client *client, wl_resource *resource, uint32_t id, const char *h)
{
    ...
    connect(exp, &XdgExportedUnstableV2Interface::unbound, s->q, [s, imp, handle]() {
    ...
    connect(imp.data(), &XdgImportedUnstableV2Interface::childChanged, s->q, [s, imp](SurfaceInterface *child) {
        ...
        connect(child, &Resource::unbound, s->q, [s, child]() {
    ...
    connect(imp.data(), &XdgImportedUnstableV2Interface::unbound, s->q, [s, handle, imp]() {
        s->importedSurfaces.remove(handle);
        Q_EMIT s->q->surfaceUnimported(handle);
...
#ifndef K_DOXYGEN
const struct zxdg_exported_v2_interface XdgExportedUnstableV2Interface::Private::s_interface = {resourceDestroyedCallback};
#endif
...
#ifndef K_DOXYGEN
const struct zxdg_imported_v2_interface XdgImportedUnstableV2Interface::Private::s_interface = {resourceDestroyedCallback, setParentOfCallback};
#endif
